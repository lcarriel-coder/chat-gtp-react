...
export const TranslatePage = () => {
  ...
  const handlePost = async (text: string, selectOption : string) => {

    setIsLoading(true);
    const newMessage = `Traduce "${ text }" al idioma ${ selectOption }`;
    setMessages((prev) => [...prev, { text: newMessage, isGpt: false }]);

    //TODO: UseCase

    const { ok, message } = await translateUseCase(text,selectOption);
